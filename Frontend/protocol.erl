...
-export_type(['Message'/0, 'Sale'/0, 'State'/0, 'User'/0]).

-spec encode_msg('Message'() | 'Sale'() | 'State'() | 'User'(), atom()) -> binary().
encode_msg(Msg, MsgName) when is_atom(MsgName) ->
    encode_msg(Msg, MsgName, []).

-spec encode_msg('Message'() | 'Sale'() | 'State'() | 'User'(), atom(), list()) -> binary().
...
encode_msg_Sale(#{} = M, Bin, TrUserData) ->
    B1 = case M of
	   #{name := F1} ->
	       begin
		 TrF1 = id(F1, TrUserData),
...
v_msg_Message(#{} = M, Path, TrUserData) ->
    ...
    case M of
      #{userType := F2} ->
	  v_type_string(F2, [userType | Path], TrUserData);
      _ -> ok
    end,
    case M of
      #{user := F3} ->
...
v_msg_Sale(#{} = M, Path, TrUserData) ->
    ...
    case M of
      #{rate := F3} ->
	  v_type_float(F3, [rate | Path], TrUserData);
      _ -> ok
    end,
...
get_msg_names() -> ['Message', 'Sale', 'State', 'User'].
